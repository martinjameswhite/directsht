...
class ThreeJ000:
    ...
    def __init__(self,Lmax=512):
        ...
        p       = np.arange(1,2*Lmax)
        lng     = np.zeros(2*Lmax)
        lng[1:] = np.cumsum(np.log( 1.-0.5/p ))
        self.gg = np.exp(lng).tolist()
        #
    def one3j(self,j1,j2,j3):
        """Evaluate a single 3-j symbol for integer js and m1=m2=m3=0."""
        J  = j1+j2+j3
        if J%2>0: return(0)
        if j3<abs(j1-j2) or j3>j1+j2: return(0)
        pp = J//2
        p1 = (-j1+j2+j3)//2
        p2 = ( j1-j2+j3)//2
        p3 = ( j1+j2-j3)//2
        gfac= self.gg[p1]*self.gg[p2]*self.gg[p3]/self.gg[pp]
        tj  = math.sqrt( gfac/(J+1.) ) # Sign ignored.
        if pp%2!=0: tj=-tj
        return(tj)
        #
